...
const ProductReviews = ({ rating, reviewer, text, date }) => {
  const firstLetter = reviewer?.charAt(0).toUpperCase();
  return (
    <div className="flex p-5 space-x-4 items-center">
      <span className="w-10 h-10 px-4 rounded-full bg-totem-pole-300 text-white font-thin flex items-center justify-center">
        {firstLetter}
      </span>
      <div className="flex flex-col">
        <h5 className="text-base font-medium mb-1">{reviewer}</h5>
        <div className="flex items-center justify-between my-2">
          {renderRatingStars(rating)}
          <span className="text-sm text-scorpion-500 ml-2">{date}</span>
        </div>
